...
void Sandbox3D::OnUpdate(Engine::Timestep ts)
{
    ...
    glm::vec3 lightDirectionView = glm::vec3(-0.2f, -1.0f, -0.3f);

    glm::vec3 forwardDirection = m_CameraController.GetCamera().GetForwardDirection();
    ...
    std::dynamic_pointer_cast<Engine::OpenGLShader>(m_Shader)->SetFloat("u_Light.constant", 1.0f);
    std::dynamic_pointer_cast<Engine::OpenGLShader>(m_Shader)->SetFloat("u_Light.linear", 0.09f);
    std::dynamic_pointer_cast<Engine::OpenGLShader>(m_Shader)->SetFloat("u_Light.quadratic", 0.032f);
